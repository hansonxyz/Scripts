...
if [ $# -lt 1 ]; then
    echo "Usage: $0 user@server [--list|--restore [--name=<session-name>]] [<initial-command>]"
    echo "Example: $0 user@server \"cd /path/to/mydir; sudo bash\""
...
# Check if the next argument is a flag.
if [[ "$1" =~ ^-- ]]; then
    FLAG=$1
    shift
fi
...
if [[ "$FLAG" == "--restore" && "$1" =~ ^--name= ]]; then
    RESTORE_SESSION="persistent_${1#--name=}"
    shift
fi

# If the initial command is not provided, use a default command.
if [ -z "$1" ]; then
    INITIAL_COMMAND="if [ -f /var/run/motd.dynamic ]; then cat /var/run/motd.dynamic; echo; fi; bash"
else
    INITIAL_COMMAND=$1
    shift
fi
